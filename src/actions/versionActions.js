...
export const getAllAppVersion = async () => {
  try {
...
export const createAppVersion = async (appName, versionName, exeFile, releaseNotesFile) => {
  try {
    const formData = new FormData();
    formData.append("appName", appName);
    formData.append("versionName", versionName);
    formData.append("applicationFiles", exeFile);
    formData.append("releaseNotes", releaseNotesFile);

    const res = await instance.post(`/app/${appName}/${versionName}`,
      formData,
      {
        headers: {
          "Content-Type": "multipart/form-data",
...
// download api
export const downloadAppById = async (id, type) => {
  try {
