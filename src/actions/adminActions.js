...
export const createRequirement = async (employeeName, employeeEmail, comments) => {
  try {
    const response = await axios.post('https://etaems.arcisai.io:5000/api/reqs/createGenerateReq', {
      employeeName: employeeName,
      employeeEmail: employeeEmail,
      comments: comments,
      employeeId: 'AD-012',
      department: 'Torque-Software',
      designation: 'Intern',
    })
    return response.data;
  } catch (error) {
...
export async function bulkAddCameraToUser(formData) {
  ...
  try {
    const response = await instance.post('/bulkAddCamerasToUser',
      formData,
    );
    return response;
  } catch (error) {
...
export const handleGetChatgptPrompt = async (page) => {
  try {
    ...
    const response = await axios.get('http://localhost:7073/api/admin/getAllChatgptPrompts', {
      params: params
    });
    return response.data;
  } catch (error) {
...
export const handleUpdateChatgptReaction = async (id, reaction) => {
  try {
    ...
    const response = await instance.put(`http://localhost:7073/api/admin/updateReactionInGpt`, null, {
...
export const getUserABDList = async (page, deviceId, limit) => {
  try {
    const params = { page, deviceId, limit };
    const response = await instance.get(`/getAbdDevice`, {
      params: params,
    });
    return response.data;
  } catch (error) {
